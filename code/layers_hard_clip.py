...
class DPSPU(nn.Module):
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            self.slope_u[mask_1] = self.slope_u[mask_1].clamp(a[mask_1], a[mask_1])
            self.slope_l[mask_1] = self.slope_l[mask_1].clamp(g_lb[mask_1], g_ub[mask_1])

            self.slope_u[mask_2] = self.slope_u[mask_2].clamp(g_ub[mask_2], g_lb[mask_2])
            self.slope_l[mask_2] = self.slope_l[mask_2].clamp(a[mask_2], a[mask_2])

            self.slope_u[mask_3] = self.slope_u[mask_3].clamp((torch.ones_like(a) * -0.25)[mask_3],
                                                               torch.maximum(torch.zeros_like(a), a)[mask_3])
            self.slope_l[mask_3] = self.slope_l[mask_3].clamp(((spu_lb + 0.5) / (lbb + epsilon))[mask_3],
                                                                g_ub[mask_3])

        lb, ub = x.lb, x.ub
        l_bias = choose_bias(lb, ub, self.slope_l)
        u_bias = choose_bias(lb, ub, self.slope_u, False)
        mat_lb = torch.cat((torch.cat((torch.diag(self.slope_l), l_bias.unsqueeze(1)), 1),
                            torch.cat((torch.zeros_like(l_bias), torch.tensor([1])), 0).unsqueeze(0)), 0)
        mat_ub = torch.cat((torch.cat((torch.diag(self.slope_u), u_bias.unsqueeze(1)), 1),
